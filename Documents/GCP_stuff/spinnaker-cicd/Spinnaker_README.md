...
################################################################################
#                                                                              #
#               Configure FIAT for Spinnaker                                   #
#                                                                              #
################################################################################

#Google groups via a G Suite Account

ADMIN=admin@gflocks.com              # An administrator's email address
...
GROUP=spinnaker-admin # The new group membership
...
################################################################################
#                                                                              #
#               Configure PUBSUB for Spinnaker                                 #
#                                                                              #
################################################################################

BUCKET=gke-test1
TOPIC=spinnaker
SUBSCRIPTION=spinnaker


BUCKET_NAME=gs://spin-gke-artifacts && PROJECT_ID=$PROJECT_ID
gsutil mb -p $PROJECT_ID $BUCKET_NAME

gsutil notification create -t $TOPIC -f json $BUCKET_NAME
...
cat > gcs-jinja.json <<EOF
[
  {
    "reference": "gs://{{ bucket }}/{{ name }}",
    "name": "gs://{{ bucket }}/{{ name }}",
    "type": "gcs/object"
  }
]
...
hal config pubsub google subscription edit --project $PROJECT_ID --json-path $SERVICE_ACCOUNT_DEST --subscription-name $SUBSCRIPTION --template-path $TEMPLATE_PATH my-google-pubsub
...
* [Spinnaker Authorization](https://www.spinnaker.io/setup/security/authorization/google-groups/)
